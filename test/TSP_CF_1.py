# TSP_7: CHRISTOFIDES ALGORITHM #1 (FAIL)
...
def christofedes(G, pos):
    opGraph = nx.DiGraph()
    #optimal_dist = 0
    # generates minimum spanning tree of graph G, using Prim's algo
    MST = genMinimumSpanningTree(G)
    odd_vert = []  # list containing vertices with odd degree
    for i in MST.nodes():
        if MST.degree(i) % 2 != 0:
            # if the degree of the vertex is odd, then append it to odd_vert list
            odd_vert.append(i)
    # adds minimum weight matching edges to MST
    minimumWeightedMatching(MST, G, odd_vert)
    ...
    while next != start:
        ...
        nx.draw_networkx_edges(G, pos, arrows=True, edgelist=[(
            curr, next)], width=2.5, alpha=0.6, edge_color='r')
    ...
    nx.draw_networkx_edges(
        G, pos, edgelist=[(curr, next)], width=2.5, alpha=0.6, edge_color='r')
    # optimal_dist = optimal_dist + G[curr][next]['length']
    # print optimal_dist
    return opGraph
