...
# Coordinates
# 0. Kuala Lumpur International Airport, Kuala Lumpur
c_KLIA_KUL = (2.7456, 101.7072)
# 1. Soekarno-Hatta International Airport, Jakarta
c_SHIA_JAK = (-6.1275, 106.6537)
# 2. Suvarnabhumi Airport, Bangkok
c_SUVA_BKK = (13.6900, 100.7501) 
# 3. Chek Lap Kok International Airport, Hong Kong
c_CLK_HKG = (22.3080, 113.9185)
# 4. Taoyuan International Airport, Taipei  
c_TAO_TPE = (25.0797, 121.2342)
# 5. Haneda International Airport, Tokyo
c_HND_TOK = (35.5494, 139.7798)  
# 6. Incheon International Airport, Seoul
c_ICN_KOR = (37.4602, 126.4407) 
# 7. Beijing Daxing International Airport, Beijing
c_BDA_PEK = (39.5098, 116.4105)
...
gmap3 = gmplot.GoogleMapPlotter(
    2.7456, 101.7072, 13, apikey="AIzaSyBU-vXpPlrm2_UBJYLzHznpvc_hhYORT8I")
...
def calc_distances(filename):
    ...
    coordinate = [c_KLIA_KUL, c_SHIA_JAK, c_SUVA_BKK,
                  c_CLK_HKG, c_TAO_TPE, c_HND_TOK, c_ICN_KOR, c_BDA_PEK]
...
def held_karp(dists):
    ...
    # Maps each subset of helthe nodes to the cost to reach that subset, as well
...
def nearest_neighbour_tsp(graph, starting_point=0):
    ...
    while number_of_nodes > len(visited_nodes):
        ...
        try:
            ...
            print(
                "Nearest Neighbour algorithm couldn't find a Neighbour that hasn't been to yet")
            return
...
error_diff = (abs(shortest_distance_NN -
                  shortest_route_dist[0])/shortest_route_dist[0]) * 100
...
def create_data_model():
    ...
    data['distance_matrix'] = [[0, (geodesic(c_KLIA_KUL, c_SHIA_JAK).km), (geodesic(c_KLIA_KUL, c_SUVA_BKK).km), (geodesic(c_KLIA_KUL, c_CLK_HKG).km), (geodesic(c_KLIA_KUL, c_TAO_TPE).km), (geodesic(
                               ...
                               [(geodesic(c_KLIA_KUL, c_SUVA_BKK).km), (geodesic(c_SHIA_JAK, c_SUVA_BKK).km), 0, (geodesic(c_SUVA_BKK, c_CLK_HKG).km), (geodesic(c_SUVA_BKK, c_TAO_TPE).km), (geodesic(
                                   c_SUVA_BKK, c_HND_TOK).km), (geodesic(c_SUVA_BKK, c_ICN_KOR).km), (geodesic(c_SUVA_BKK, c_BDA_PEK).km)],  # Suvarnabhumi Airport, Bangkok
                               [(geodesic(c_KLIA_KUL, c_CLK_HKG).km), (geodesic(c_SHIA_JAK, c_CLK_HKG).km), (geodesic(c_SUVA_BKK, c_CLK_HKG).km), 0, (geodesic(c_CLK_HKG, c_TAO_TPE).km), (geodesic(
                                   c_CLK_HKG, c_HND_TOK).km), (geodesic(c_CLK_HKG, c_ICN_KOR).km), (geodesic(c_CLK_HKG, c_BDA_PEK).km)],  # Chek Lap Kok International Airport, Hong Kong
                               [(geodesic(c_KLIA_KUL, c_TAO_TPE).km), (geodesic(c_SHIA_JAK, c_TAO_TPE).km), (geodesic(c_SUVA_BKK, c_TAO_TPE).km), (geodesic(c_CLK_HKG, c_TAO_TPE).km), 0, (geodesic(
                                   c_TAO_TPE, c_HND_TOK).km), (geodesic(c_TAO_TPE, c_ICN_KOR).km), (geodesic(c_TAO_TPE, c_BDA_PEK).km)],  # Taoyuan International Airport, Taipei
                               [(geodesic(c_KLIA_KUL, c_HND_TOK).km), (geodesic(c_SHIA_JAK, c_HND_TOK).km), (geodesic(c_SUVA_BKK, c_HND_TOK).km), (geodesic(c_CLK_HKG, c_HND_TOK).km), (geodesic(
...
def print_solution(manager, routing, solution):
    ...
    cities_list = ["KUL", "JAK", "BKK", "HKG", "TPE", "TOK", "KOR", "PEK"]
    print('Shortest distance    : {} km (approximate)'.format(
    ...
    shortestPath_nodeIndex += '{} (and vice versa)'.format(
        manager.IndexToNode(index))
    shortestPath_nodeName += '{} (and vice versa)\n'.format(
        cities_list[sourceCityIndex])
...
labels = ('Jakarta', 'Bangkok', 'Hong Kong',
...
plt.scatter(y_pos, wordCount_before, color=[
            'black', 'red', 'green', 'blue', 'cyan', 'yellow', 'orange'])
plt.scatter(y_pos, wordCount_after, color=[
            'black', 'red', 'green', 'blue', 'cyan', 'yellow', 'orange'])
...
# Word sets
# JAKARTA:
jak_pos = ['growth', 'grow', 'grew', 'optimistic', 'sufficient', 'highest', 'higher', 'expanded', 'rise', 'rising',
           'high-income', 'easing', 'productive', 'supporting', 'supported', 'equitable', 'maintain', 'stable', 'better', 'advantage']
jak_neg = ['shave', 'downturn', 'fell', 'slowdown', 'lowest', 'risk', 'fall', 'weakening', 'weaken', 'deficit',
           'worsen', 'worst', 'burden', 'stress', 'downfall', 'decline', 'risky', 'risks', 'declining', 'declined']

# BANGKOK:
bkk_pos = ['normal', 'maintain', 'strong', 'plan', 'plans', 'positive', 'increased', 'spike', 'rise', 'sufficient',
           'stable', 'reliable', 'manageable', 'surged', 'spiked', 'soared', 'rose', 'relief', 'expand', 'accelerated']
bkk_neg = ['risks', 'irregularity', 'falling', 'weak', 'weakest', 'damage', 'damaging', 'reeling', 'collapse',
           'drop', 'loss', 'losses', 'slowdown', 'risk', 'negative', 'critical', 'slower', 'fell', 'stop', 'fall']

# HONG KONG:
hkg_pos = ['rise', 'maintain', 'supporting', 'stability', 'recover', 'bounce', 'restore', 'save', 'fund', 'boost',
           'stimulating', 'increase', 'increased', 'rebooting', 'developing', 'develop', 'relieve', 'grow', 'better', 'stimulate']
hkg_neg = ['deteriorating', 'falling', 'worse', 'weakening', 'slowing', 'unemployment', 'threat', 'deficit', 'risk',
           'reduced', 'lower', 'weakening', 'crisis', 'recession', 'downturn', 'unrest', 'fell',  'slowing', 'reeling', 'critical']

# TAIPEI:
tpe_pos = ['ramped', 'authorised', 'growing', 'expand', 'increase', 'sufficient', 'secure', 'high', 'shoring', 'stability',
           'revive', 'maintaining', 'soared', 'rise', 'rising', 'increased', 'surged', 'strong',  'maintain', 'revised']
tpe_neg = ['lowered', 'downgrade', 'uncertainty', 'cut', 'crisis', 'canceled', 'fallen', 'losses', 'subsides',
           'slow', 'low', 'recession', 'worst', 'interrupt', 'reduced', 'fell', 'dropped', 'plunge', 'lower', 'downgraded']

# TOKYO:
tok_pos = ['deploy', 'raising', 'raised', 'raises', 'greater', 'recovering', 'expanded', 'improvement', 'improve', 'expansion',
           'expanding', 'forestall', 'funded', 'protected', 'overcome', 'helped', 'upgrades', 'improved', 'recouped', 'recovery']
tok_neg = ['stagnation', 'crisis', 'stagnate', 'risks', 'recession', 'losses', 'hurt', 'loss', 'cancellation',
           'worse', 'risk', 'conflicts', 'worsens', 'delayed', 'negative', 'lower', 'sicker', 'bad', 'drop', 'disruption']

# SEOUL:
kor_pos = ['recovery', 'raise', 'prevention', 'recover', 'rebound', 'improved', 'efforts', 'stronger', 'additional', 'rose',
           'expansionary', 'ensure', 'helped', 'stabilization', 'improving', 'expended', 'improvement', 'sufficiently', 'substantial', 'gain']
kor_neg = ['suffer', 'suffered', 'recession', 'severe', 'setbacks', 'suspended', 'warns', 'delaying', 'delayed',
           'shutdowns', 'damage', 'critical', 'fallout', 'dropped', 'problematic', 'crisis', 'negatively', 'hit', 'worse', 'falling']

# BEIJING:
pek_pos = ['growth', 'recovery', 'supporting', 'increasing', 'stabilizing', 'maintained', 'grow', 'grew', 'improvement', 'upwards',
           'boost', 'upgraded', 'achieving', 'stability', 'increasing', 'sustainably', 'expand', 'recovery', 'strengthened', 'positively']
pek_neg = ['decline', 'collapse', 'imbalance', 'dropped', 'fell', 'debt', 'contract', 'drop', 'bottom', 'weak',
           'suffered', 'stressed', 'risk', 'scandals', 'degradation', 'tension', 'declines', 'difficult', 'reducing', 'weaker']

# 2D Array: [posWords, negWords] for all 7 cities
posnegArr = [[jak_pos, jak_neg], [bkk_pos, bkk_neg], [hkg_pos, hkg_neg], [
    tpe_pos, tpe_neg], [tok_pos, tok_neg], [kor_pos, kor_neg], [pek_pos, pek_neg]]
...
posnegFrequencyArr = [[jak_pos_f, jak_neg_f], [bkk_pos_f, bkk_neg_f], [hkg_pos_f, hkg_neg_f], [
    tpe_pos_f, tpe_neg_f], [tok_pos_f, tok_neg_f], [kor_pos_f, kor_neg_f], [pek_pos_f, pek_neg_f]]
...
for i in range(0, len(filesArr), 1):
    ...
    for j in posnegArr:
        ...
        for k in words:
            ...
            if k in posnegArr[index][index2]:
                ...
                posnegFrequencyArr[index][index2][(posnegArr[index][index2]).index(
                    k)] = posnegFrequencyArr[index][index2][(posnegArr[index][index2]).index(k)] + 1
    ...
    print("Number of positive words in {0}: {1}".format(
        filesArr[index], posnegCountArr[index][0]))
    print("Positive word sets:\n{0}\nFrequency:\n{1}\n".format(
        posnegArr[index][0], posnegFrequencyArr[index][0]))

    print("Number of negative words in {0}: {1}".format(
        filesArr[index], posnegCountArr[index][1]))
    print("Negative word sets:\n{0}\nFrequency:\n{1}\n".format(
        posnegArr[index][1], posnegFrequencyArr[index][1]))
    ...
    print("Percentage of positive words in {}: {:.4f}%".format(
        filesArr[index], posPercent))
    print("Percentage of negative words in {}: {:.4f}%\n".format(
        filesArr[index], negPercent))
...
pos_fullSet = [jak_pos_set, bkk_pos_set, hkg_pos_set,
               tpe_pos_set, tok_pos_set, kor_pos_set, pek_pos_set]

neg_fullSet = [jak_neg_set, bkk_neg_set, hkg_neg_set,
               tpe_neg_set, tok_neg_set, kor_neg_set, pek_neg_set]

cityName = ["Jakarta", "Bangkok", "Hong Kong",
            "Taipei", "Tokyo", "Seoul", "Beijing"]
...
for i in range(len(cityName)):
    ...
    # Positive word count
    plt.title("Positive Word Frequency for {}".format(
        cityName[i]), fontsize=15)
    ...
    # Negative word count
    plt.title("Negative Word Frequency for {}".format(
        cityName[i]), fontsize=15)
    plt.xlabel("Frequency of words", fontsize=13)
    plt.ylabel("List of words", fontsize=13)
...
yvals = [posnegCountArr[0][0], posnegCountArr[1][0], posnegCountArr[2][0],
         posnegCountArr[3][0], posnegCountArr[4][0], posnegCountArr[5][0], posnegCountArr[6][0]]
rects1 = ax.bar(ind, yvals, width, color='g')
zvals = [posnegCountArr[0][1], posnegCountArr[1][1], posnegCountArr[2][1],
         posnegCountArr[3][1], posnegCountArr[4][1], posnegCountArr[5][1], posnegCountArr[6][1]]
...
ax.set_xticklabels(('Jakarta', 'Bangkok', 'Hong Kong',
                    'Taipei', 'Tokyo', 'Seoul', 'Beijing'))
...
cities = ['Jakarta', 'Bangkok', 'Hong Kong',
          'Taipei', 'Tokyo', 'Seoul', 'Beijing'] 
...
for i in range(len(cities)):
    ...
    print("Positive sentiment: {:.4f}%\tNegative sentiment: {:.4f}%".format(
        posPercentArr[index], negPercentArr[index]))
    ...
    if (posPercentArr[index] > negPercentArr[index]):
        ...
        print("Since {:.4f}% > {:.4f}%, {} has positive sentiment.\n".format(
            posPercentArr[index], negPercentArr[index], cities[index]))
...
def CalcDist(st, pt, sh):
    ...
    print("Distance from {} to {}: {} km".format(
        Shortest[pt], Shortest[sh], d2))
...
DShort = [c_SHIA_JAK, c_CLK_HKG, c_TAO_TPE,
          c_HND_TOK, c_ICN_KOR, c_BDA_PEK,  c_SUVA_BKK]

# Sorted location of Sentiment List
DSent = [c_SHIA_JAK, c_BDA_PEK, c_HND_TOK,
         c_SUVA_BKK,  c_CLK_HKG, c_ICN_KOR, c_TAO_TPE]
...
def distance(allPath):
    ...
    for i, x in enumerate(allPath):
        if i == 0:
            ...
            totalDist += geodesic(coordinate[allPath[i-1]],
                                  coordinate[allPath[i]]).km
...
def sentimentMeasure(l1, l2, path, dist):
    ...
    for x, y in enumerate(l2):
        ...
        if l1[x] != y:
            ...
            # ? Route sentiment = -ve && Moved to the left (less recommended)
            # * Penalty = -2 * number of spots moved
            ...
            # ? Route sentiment = -ve && Moved to the right (more recommended)
            # * Penalty = -1 * number of spots moved
            ...
            # ? Route sentiment = +ve && Moved to the left (more recommended)
            # * Penalty = -1 * number of spots moved
            elif y > 0 and l2.index(y) < l1.index(y):
                # sent_measure.append(-1 * (abs(l2.index(y) - l1.index(y))))
                sent_measure.append([y, 1 * (abs(l2.index(y) - l1.index(y)))])
            ...
            # ? Route sentiment = +ve && Moved to the right (less recommended)
            # * Penalty = -2 * number of spots moved
            elif y > 0 and l2.index(y) > l1.index(y):
                # sent_measure.append(-2 * (abs(l2.index(y) - l1.index(y))))
                sent_measure.append([y, 2 * (abs(l2.index(y) - l1.index(y)))])
...
for p, v in enumerate(path):
    ...
    pathName.append(['KUL', cityName[path[p][1]-1], cityName[path[p][2]-1], cityName[path[p][3]-1],
                     cityName[path[p][4]-1], cityName[path[p][5]-1], cityName[path[p][6]-1], cityName[path[p][7]-1], 'KUL'])
    ...
    file2.write("Path [{0:04d}]\t:\t{1}\nDistance\t:\t{2} km\nSentiment\t:\t{3}\n\n".format(
        p+1, pathName[p], distList[p], sentimentRoutes[p]))

file2.close()

totalSMArr = []*5040
for i in range(len(sentimentRoutes)):
    totalSMArr.append(sentimentMeasure(
        sentBest, sentimentRoutes[i], pathName[i], distList[i]))

sortedTotalSMArr = sorted(
    totalSMArr, key=lambda x: (-x[1], x[0]), reverse=True)

for i in range(len(sentimentRoutes)):
    file3.write("Path [{0:04d}]\t\t:\t{1}\nDistance\t\t:\t{2} km\nSentiment\t\t:\t{3}\nTotal Sentiment Measure\t:\t{4}\n\n".format(
        i+1, sortedTotalSMArr[i][2], sortedTotalSMArr[i][3], sortedTotalSMArr[i][0], sortedTotalSMArr[i][1]))
...
for y, z in enumerate(totalSMArr):
    file4.write("Path [{0:04d}]\t\t:\t{1}\nDistance\t\t:\t{2} km\nSentiment\t\t:\t{3}\nTotal Sentiment Measure\t:\t{4}\n\n".format(
        y+1, temp[y][2], temp[y][3], temp[y][0], temp[y][1]))
...
plt.xticks(index + bar_width/2, ('Shortest Distance',
                                 'Best Sentiment', 'Optimised'))
